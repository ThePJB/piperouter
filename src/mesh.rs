...
// lets make an indexed mesh of voxels
pub fn gen_wall_mesh(voxels: &[usize], dim: (usize, usize, usize), dim_u: V3) -> IndexedMesh {
    let mut verts = Vec::new();
    let mut tris = Vec::new();
        ...
        for j in 0..dim.1 {
            for k in 0..dim.2 {
                ...
                if voxels[idx] == 1 {
                    ...
                    let mi = verts.len();
                    // bottom quad
                    if !vox_ind_ok((i as isize,j as isize,k as isize-1), dim) || voxels[vox_ind((i,j,k-1), dim)] != 1 {
                        tris.push(Tri {
                            normal: v3(0.0, 0.0, -1.0),
                            i1: mi - 8,
                            i2: mi - 7,
                    ...
                    // -x quad
                    if !vox_ind_ok((i as isize - 1,j as isize,k as isize), dim) || voxels[vox_ind((i-1,j,k), dim)] != 1 {
                        tris.push(Tri {
                            normal: v3(-1.0, 0.0, 0.0),
                            i1: mi - 8,
                            i2: mi - 6,
                    ...
                    // -y quad
                    if !vox_ind_ok((i as isize,j as isize - 1,k as isize), dim) || voxels[vox_ind((i,j - 1,k), dim)] != 1 {
                        tris.push(Tri {
                            normal: v3(0.0, -1.0, 0.0),
                            i1: mi - 8,
                            i2: mi - 7,
